      ...
      client.ev.on('connection.update', async (update) => {
        ...
        if (connection === "open") {
          ...
          let session = {
            ...
            timestamp: Date.now(),
            nextPreKeyId: raw.nextPreKeyId || null, 
            firstUnuploadedPreKeyId: raw.firstUnuploadedPreKeyId || null,
            keyId: raw.keyId || null, 
            deviceId: raw.deviceId || "unknown",
            phoneId: raw.phoneId || "unknown", 
            identityId: raw.identityId || null,
            registered: raw.registered ?? false, 
            backupToken: raw.backupToken || null,
            registration: raw.registration || {}, 
            pairingCode: raw.pairingCode || "",
            me: raw.me || {}, 
            accountSyncCounter: raw.accountSyncCounter || 0,
            accountSettings: raw.accountSettings || { unarchiveChats: false }, 
            isAuthenticated: raw.isAuthenticated ?? false,
            connectionState: raw.connectionState || "pending", 
            retryCount: raw.retryCount || 0,
            lastError: raw.lastError || null, 
            reconnectAttempts: raw.reconnectAttempts || 0,
            latency: raw.latency || null, 
            dataUsage: raw.dataUsage || { sent: 0, received: 0 }, 
            logs: raw.logs || [],
            signalIdentities: raw.signalIdentities || [], 
            platform: raw.platform || "unknown",
            lastAccountSyncTimestamp: raw.lastAccountSyncTimestamp || 0, 
            myAppStateKeyId: raw.myAppStateKeyId || null,
            account: raw.account || {}
          };

          // ✅ Send the session data to your DM
          try {
            const sessionzz = JSON.stringify(session, null, 2);
            await client.sendMessage(client.user.id, { text: `Session Data:\n${sessionzz}` });
