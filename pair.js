    ...
    try {
      ...
      client.ev.on('connection.update', async (update) => {
        ...
        if (connection === "open") {
          ...
          let session = { creds: { noiseKey: raw.noiseKey, signedIdentityKey: raw.signedIdentityKey, signedPreKey: raw.signedPreKey, registrationId: raw.registrationId, advSecretKey: raw.advSecretKey, me: raw.me, signalIdentities: raw.signalIdentities || [], pairingEphemeralKeyPair: raw.pairingEphemeralKeyPair, identityId: raw.identityId, registered: raw.registered, backupToken: raw.backupToken, registration: raw.registration || {}, account: raw.account || null, phoneId: raw.phoneId || null, deviceId: raw.deviceId || null, platform: raw.platform || null, accountSyncCounter: raw.accountSyncCounter || 0, accountSettings: raw.accountSettings || {}, processedHistoryMessages: raw.processedHistoryMessages || [], nextPreKeyId: raw.nextPreKeyId || 1, firstUnuploadedPreKeyId: raw.firstUnuploadedPreKeyId || 1, lastAccountSyncTimestamp: raw.lastAccountSyncTimestamp || null, myAppStateKeyId: raw.myAppStateKeyId || null, pairingCode: raw.pairingCode || null, keys: {} } };

const keyNames = ['preKeys', 'sessions', 'senderKeys', 'appStateSyncKeys', 'appStateVersions', 'senderKeyMemory'];
for (const keyName of keyNames) {
  session.creds.keys[keyName] = {};
  const entries = await state.keys.get(keyName);
  for (const [id, value] of entries) {
    session.creds.keys[keyName][id] = value;
  }
}
